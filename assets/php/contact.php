  ...
  if($_SERVER['REQUEST_METHOD'] != 'POST') {
    ...
    if (empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) != 'xmlhttprequest') {
      ...
      if (empty($EMAIL) || empty($NAME) || empty($SUBJECT) || empty($HTML)) {
        die(json_encode(array('type' => 'error', 'msg' => '<span class="form-icon-error"></span> An error occurred. Please check php config field.')));
      } else {
        ...
        if (!$mail->send()) {
          die(json_encode(array('type' => 'error', 'msg' => '<span class="form-icon-error"></span> An error occurred. Please try again later.')));
        } else {
          die(json_encode(array('type' => 'success', 'msg' => '<span class="form-icon-success"></span> Your message has been sent.')));
        }
      }
    }
  }
